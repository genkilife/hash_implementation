...
int main(){
	...
	*/
	fstream fs;
	fs.open("result/cuckooHash_collide_insert_timestamp.txt", fstream::out);
	...
	fs.open("result/cuckooHash_collide_search_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
	...
	/*
		Dump into result file.
	*/
	fs.open("result/cuckooHash_collide_delete_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
