...
int main(){
		...
		if((idx % SAMPLE_PERIOD) == 0){
			...
			if(idx > 0){
				...
				for(unsigned iidx=idx-SAMPLE_PERIOD; iidx < idx; iidx++){
					if(dhPtr->search(keys[iidx]) != SUCCESS){
						assert(false);
					}
				}
	...
	*/
	fstream fs;
	fs.open("result/doubleHash_collide_insert_timestamp.txt", fstream::out);
	...
	fs.open("result/doubleHash_collide_search_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
	...
	/*
		Dump into result file.
	*/
	fs.open("result/doubleHash_collide_delete_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
