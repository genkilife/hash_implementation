...
int main(){
	...
	*/
	fstream fs;
	fs.open("result/doubleHash_uniform_insert_timestamp.txt", fstream::out);
	...
	fs.open("result/doubleHash_uniform_search_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
	...
	/*
		Dump into result file.
	*/
	fs.open("result/doubleHash_uniform_delete_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < keySize/SAMPLE_PERIOD; idx++){
